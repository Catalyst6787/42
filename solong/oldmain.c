...
int	main(void)
{
	...
	img.nimg = mlx_new_image(mlx, 1920, 1080);
	img.naddr = mlx_get_data_addr(img.img, &img.bits_per_pixel, &img.line_length, &img.endian);
	my_mlx_pixel_put(&img, 5, 11, 0xf9f9f9);
	my_mlx_pixel_put(&img, 5, 13, 0xf9f9f9);
	my_mlx_pixel_put(&img, 5, 15, 0xf9f9f9);
	mlx_put_image_to_window(mlx, mlx_win, img.nimg, 0, 0);
